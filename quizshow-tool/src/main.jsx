...
function AudioInput({ value, name, onChange }) {
  ...
  return <div className="audio-input">
    <button type="button" onClick={() => ref.current.click()}><Upload /> Timer-Melodie wählen</button>
...
function PlayerPage({ code }) {
  ...
  function submitText() {
    ...
    if (!window.confirm('Willst du diese Antwort wirklich abgeben?')) return;
    patch(r => {
  ...
  function vote(answerId) {
    ...
    const label = answer?.letter ? `Antwort ${answer.letter}` : 'diese Antwort';
    if (!window.confirm(`Wirklich für ${label} voten?`)) return;
    patch(r => {
  ...
  return <div className={`player-page player-res-${viewMode}`}>
    <ShowLayout room={room} mode="player" onVote={vote} activePlayerId={playerId} viewMode={viewMode} onToggleViewMode={toggleViewMode} playerControls={playerControls} />
  </div>
}
...
function ShowLayout({ room, mode, onVote, activePlayerId, viewMode, onToggleViewMode, playerControls }) {
  ...
  return <div className={`show-layout ${mode} ${isPlayerWriting ? 'player-writing' : ''}`}>
    ...
    <section className="answers-panel">
      ...
      {isPlayerWriting ? <><TimerDisplay room={room} /><PlayerWritingPanel controls={playerControls} /></> : room.current.answers.length ? room.current.answers.slice(0, room.current.visibleAnswerCount ?? room.current.answers.length).map(answer => <AnswerCard key={answer.id} answer={answer} room={room} onVote={onVote} canVote={mode === 'player' && room.current.activeVoterId === activePlayerId} />) : <><div className="waiting-card">Warte auf Antworten…</div><TimerDisplay room={room} /></>}
    </section>
  </div>
}
...
function PlayerWritingPanel({ controls }) {
  return <div className="player-writing-panel">
    ...
    <button className="primary" disabled={!controls.canSubmit} onClick={controls.submitText}>{controls.isEditing ? 'Korrektur erneut absenden' : controls.hasSubmitted ? 'Antwort abgegeben' : 'Text abgeben'}</button>
